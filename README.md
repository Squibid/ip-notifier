...
### Running the program
Once you have cloned the repository you need to run the install script by running `bash install.sh` once that is done running the program is ready and you can start it with `bash main.sh` if there is a problem please open an [issue](https://github.com/Squibid/ip-notifier/issues) so I can fix the problem.
